...
class GuildStoredMessage extends GuildMessage {
	 ...
	 */
	public function __construct(string $guildId, string $id, string $channelId, string $content, User $author, ?GuildReceivedEmbed $embed = null, string $timestamp = "", bool $tts = false, bool $pinned = false, ?array $referenced_message = null, array $attachments = [], array $components = [], ?string $edited_timestamp = null, int $type = 0, int $flags = 0, bool $mention_everyone = false, array $mentions = [], array $mention_roles = []) {
		$this->author = $author;
		if (is_array($referenced_message)) $referenced_message = MessageInitializer::fromStore($guildId, $referenced_message);
		
		parent::__construct($guildId, $id, $channelId, $content, null, $embed, $timestamp, $tts, $pinned, $referenced_message, $attachments, $components, $edited_timestamp, $type, $flags, $mention_everyone, $mentions, $mention_roles);
	}
